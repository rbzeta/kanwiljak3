...
function submitSave(form,select_kanca,
	...
	if(insertAtmMasterValidation(form,select_kanca,
			masteratm_branch_code,
			masteratm_tid,
			select_tipeatm,
			masteratm_lokasi,
		...
		if(confirm('Anda yakin ingin menyimpan?')){
		    ...
		    saveaction.value = "input_master_atm";
...
if (!empty($_POST["saveaction"])){
	...
	$insert_prep = "INSERT INTO m_master_atm (
					 masteratm_branch_mbcode,
					  masteratm_branch_code,
					  masteratm_tid,
					  masteratm_tipe,
	...
	if ($insert_stmt) {
		...
		$insert_stmt->bind_param('iisisissiiissiiiii', 	validateInput($_POST['select_kanca']),
													...
													$isActive);//default value for active is 1
	...
	<table width="100%"  border="0" cellpadding="00">
      <tr>
        <td align="center" valign="top">
			<div id="tabs">
				<ul>
					<li><a href="#tabs-1" style="font-size:12px;"><b>Input Data</b></a></li>
