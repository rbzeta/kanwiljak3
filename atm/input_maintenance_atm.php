...
function submitSave(form,atm_act_date,
		atm_act_tid,
		select_merk,
		select_cro,
		select_isonsite,
	...
	if(insertAtmMaintenanceValidation(form,atm_act_date,
			atm_act_tid,
			select_merk,
			select_cro,
			select_isonsite,
		...
		if(confirm('Anda yakin ingin menyimpan?')){
		    ...
		    saveaction.value = "input_maintenance_atm";
...
if (!empty($_POST["saveaction"])){
	...
	$insert_prep = "INSERT INTO m_atm_activitylog
					            (atm_act_date,
					             atm_act_tid,
					             atm_act_brand_id,
					             atm_act_branch_id,
	...
	if ($insert_stmt) {
		...
		$insert_stmt->bind_param('ssiisisiisisiissss', 
...
if ($kanca_stmt) {
	$atm_act_branch_id = $_SESSION['user_branch_id'];
	
	if ($isUker)$kanca_stmt->bind_param('i', $atm_act_branch_id);
...
if ($team_stmt) {
	$atm_act_pmteamkw_id = $_SESSION['user_id'];
	$team_stmt -> bind_param('i', $atm_act_pmteamkw_id);
	$team_stmt->execute();
