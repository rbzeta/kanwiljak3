...
function submitSave(form,recordId,
	...
	if(updateUkoMaintenanceValidation(form,recordId,
			ukermaintenance_date,
			select_kanca,
			ukermaintenance_keterangan,
			ukermaintenance_problem,
		...
		if(confirm('Anda yakin ingin menyimpan?')){
		    ...
		    saveaction.value = "update_maintenance_uko";
...
if (!empty($_POST["saveaction"])){
	...
	$insert_prep = "UPDATE m_uker_maintenance
					SET 		ukermaintenance_date = ?,
					             ukermaintenance_branch_id = ?,
					             ukermaintenance_problem = ?,
					             ukermaintenance_tindaklanjut = ?,
					             ukermaintenance_status_id = ?,
	...
	if ($insert_stmt) {
		...
		$insert_stmt->bind_param('sissississi', 
...
$select_prep = "SELECT ukermaintenance_id,
				...
				WHERE ukermaintenance_id = ? LIMIT 1";
...
if ($select_stmt->num_rows >= 1) {
	...
	$varDate = str_replace('/', '-', $ukermaintenance_date);
	$txtDate = date('d/m/Y', strtotime($varDate));
	$ukermaintenance_date = $txtDate;
}
?>
<div class="right">
	...
	<tr>
		<td valign="top" width="50%">
			<table width="100%" border="0" cellpadding="1" cellspacing="2">
				...
				<tr>
					<td class="list_left">KC Supervisi <?php echo $ukermaintenance_branch_id?></td>
					<td class="list_right">: 
