...
function updateReportDetilByTID(recId){
    ...
    action.name = "action";
    action.type = "hidden";
    action.value = "opentidmerchantinputedit";
...
function deleteAction(recId){
	if(confirm('Anda yakin ingin menghapus data ?')){
	    ...
	    actionType.value = "openmenumerchantinputdelete";
...
if (!empty($_POST["deleteaction"])){
	...
	$delete_prep = "UPDATE m_edc_merchant SET edcmerchant_isactive = 0 WHERE edcmerchant_id = ?  ";
...
<div class="right">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
  		<tr>
        	...
        	<table width="100%"  border="0" cellspacing="0" cellpadding="0">
         	 	<tr>
             	 <td align="center" valign="middle"><h3 style="margin-top:0px;"><b>LIST EDC MERCHANT</b></h3></td>
          		</tr>
        	</table>
    	 </td>
  		</tr>
	              			...
	              			<tr>
								<td align="left" valign="middle">
									...
									<?php 
										...
										$orderbySQL = " ORDER BY ";
										$orderby = " edcmerchant_kodekanca,edcmerchant_tid ";
										$ascdesc = " asc ";
										...
										$select_prep = "SELECT edcmerchant_id,edcmerchant_tid,edcmerchant_mid,edcmerchant_tipe,edcmerchant_kodekanca,edcmerchant_nama,edcmerchant_brand,
															...
															edcmerchant_pic,edcmerchant_pictelp,edcbrand_id,edcbrand_nama,
															mainbr as branch_mbcode,brdesc as branch_mbname,branch as branch_code,brdesc as branch_name
														FROM m_edc_merchant
														LEFT JOIN m_edc_brand ON edcbrand_id = edcmerchant_brand
														LEFT JOIN dwh_branch ON branch = edcmerchant_kodekanca 
														WHERE edcmerchant_isactive = 1 ";
											...
											if (validateInput($_POST["txtSearch"]) <> "") {
												...
												switch ($post) {
													case "edcmerchant_tid":
														$criteria .= " AND edcmerchant_tid LIKE ? ";
														break;
													case "edcmerchant_mid":
														$criteria .= " AND edcmerchant_mid LIKE ? ";
														break;
													case "kodeuker":
													...
													case "edcmerchant_nama":
														$criteria .= " AND edcmerchant_nama LIKE ? ";
														break;
													case "edcmerchant_alamat":
														$criteria .= " AND edcmerchant_alamat LIKE ? ";
													break;
													default:
														;
														break;
												...
												if ($logged == "in") {
													if ($isUker) {
														$criteria .= " and edcmerchant_kodekanca IN (select branch from dwh_branch where MAINBR = ".$_SESSION['user_branch_code'].") ";
													}
												}
												...
												if ($logged == "in") {
													if ($isUker) {
														$select_prep .= " AND edcmerchant_kodekanca IN (select branch from dwh_branch where MAINBR = ".$_SESSION['user_branch_code'].") ";
														$count_prep .= " AND edcmerchant_kodekanca IN (select branch from dwh_branch where MAINBR = ".$_SESSION['user_branch_code'].") ";
													}
												}											
											}
...
if ($logged == "in") {
	$strActionScript .= '<td style="text-align: center;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">
						<a class="underline" href="javascript:updateReportDetilByTID('.$edcmerchant_tid.');"><img src="../resource/pen.png" width="11" title="edit" alt="edit" /></a>
						<a class="underline" href="javascript:deleteAction('.$edcmerchant_id.');"><img src="../resource/delete_icon.png" width="11" title="delete" alt="delete" /></a>
				</td>';
}
				...
				href="javascript:openReportDetilByTID(this.form,'.$edcmerchant_tid.');" >'.$edcmerchant_tid.'</a></td>
											...
											<td style="text-align: left;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">'.$edcmerchant_tipe.'</td>
											...
											<td style="text-align: left;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">'.$edcmerchant_pictelp.'</td>';	
									$rNum += 1;
								}
							}else $msg .= '<tbody class="table-hover"><tr class="tr"><td colspan="8" style="text-align: center;" class="td">Data tidak ditemukan</td></tr></tbody>';
