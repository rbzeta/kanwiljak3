...
<div class="right">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
  		<tr>
        	...
        	<table width="100%"  border="0" cellspacing="0" cellpadding="0">
         	 	<tr>
             	 <td align="center" valign="middle"><h3 style="margin-top:0px;"><b>MAINTENANCE EDC</b></h3></td>
          		</tr>
        	</table>
    	 </td>
  		</tr>
								...
								<td align="left" valign="middle">
									...
									<?php 
										...
										$select_prep = " SELECT edcmaintenance_id,
																...
																status_name,
																branch_name
														FROM m_edc_maintenance
														LEFT JOIN m_status ON status_id = edcmaintenance_status_id
														LEFT JOIN m_branch ON branch_id =  edcmaintenance_branch_id";
										
										$count_prep = "SELECT COUNT(edcmaintenance_id)
														FROM m_edc_maintenance 
														LEFT JOIN m_status ON status_id = edcmaintenance_status_id
														LEFT JOIN m_branch ON branch_id =  edcmaintenance_branch_id";
										...
										if(!empty($_POST["actioncari"]) || !empty($action)){
											if (validateInput($_POST["txtSearch"]) <> "") {
												...
												switch ($post) {
													case "edcmaintenance_tid":
														$criteria .= " WHERE edcmaintenance_tid LIKE ? ";
														break;
													case "edcmaintenance_petugas":
														$criteria .= " WHERE edcmaintenance_petugas LIKE ? ";
														break;
													default:
														;
														break;
												...
												if ($logged == "in") {
													if ($isUker) {
														$criteria .= " and branch_code = ".$_SESSION['user_branch_code'];
													}
												}
												...
												if ($logged == "in") {
													if ($isUker) {
														$select_prep .= " where branch_code = ".$_SESSION['user_branch_code'];
														$count_prep .= " where branch_code = ".$_SESSION['user_branch_code'];
													}
												}											
											}
...
//  									echo $count_prep.$logged.$isUker;
									...
									<option value="edcmaintenance_tid" <?php if ($post == "edcmaintenance_tid") { echo "selected"; }else echo "";?>>TID</option>
									<option value="edcmaintenance_petugas" <?php if ($post == "edcmaintenance_petugas") { echo "selected"; }else echo "";?>>Nama</option>
									</select>
								...
								<td align="right">
									<b><i>Total Data Maintenance EDC: <?php echo number_format($totalatm,0);?></i></b>
								</td>
	              			</tr>
						...
						try {	
							$rNum = ($per_page * ($page)) + 1;
 							...
 							if ($select_stmt->num_rows >= 1) {
								...
								$select_stmt->bind_result($edcmaintenance_id,
														$edcmaintenance_tid,
														$edcmaintenance_keterangan,
														$edcmaintenance_jenis_id,
														$edcmaintenance_date,
								...
								while ($select_stmt->fetch()){
									...
									$msg .= '<tbody class="table-hover"><tr class="tr">
											...
											<td style="text-align: left;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">'.date("Y-m-d",strtotime($edcmaintenance_date)).'</td>
											<td style="text-align: center;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">
											<a style="font-size: 13px;font-family: Courier New;" href="javascript:openReportDetilByTID(this.form,'.$edcmaintenance_tid.');" >'.$edcmaintenance_tid.'</a></td>
											...
											<td style="text-align: left;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td">'.$edcmaintenance_keterangan.'</td>					
							 				<td style="text-align: left;font-size: 13px;padding-left:7px;padding-right:7px;font-family: Courier New;" nowrap class="td" width="100px">'.$edcmaintenance_petugas.'</td>';
